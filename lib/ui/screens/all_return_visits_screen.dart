...
class AllReturnVisitsScreen extends StatelessWidget {
  ...
  Consumer<AllReturnVisitsListModel> _buildBody() {
    return Consumer<AllReturnVisitsListModel>(
        builder: (context, model, __) {
          if(model.hasItems) {
            return GroupedReturnVisitListView(
              collection: model.groupedCollection);
          } else {
            return Center(child: Text("No Return Visits."),);
          }
        },
      );
  }
    ...
    return PreferredSize(
      ...
      child: Padding(
        padding: EdgeInsets.only(
            ...
            right: 56.0 + (AppStyles.leftMargin * 2)),
        child: Consumer<AllReturnVisitsListModel>(
      builder: (context, model, __) => SearchWidget<ReturnVisitListItemModel>(
        ...
        textFieldBuilder: (controller, focusNode) {
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(width: 1.00, color: Color(0xffd9d9d9),), 
            boxShadow: [
              BoxShadow(
        ...
        popupListItemBuilder: (item) => Padding(
          ...
          child: GestureDetector(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text(item.nameOrDescription, style: AppStyles.smallTextStyle.copyWith(fontWeight: FontWeight.bold),),
                Text(item.formattedAddress, style: AppStyles.smallTextStyle,)
              ],
            ),
          ),
        ),
  ...
  PreferredSize _buildPinnedReturnVisitsWidget(BuildContext context, {UnmodifiableListView<ReturnVisitListItemModel> items}) {
    return PreferredSize(
        ...
        MaterialPageRoute(
          builder: (_) => GenericCollectionScreen(
            ...
            items: items, 
            widgetBuilder: (m) => Padding(
              padding: EdgeInsets.only(left: AppStyles.leftMargin, right: AppStyles.leftMargin, bottom: 17.0),
              child: ReturnVisitCard(returnVisit: m,),
            ),
