...
class GroupedReturnVisitByNameCollection
  ...
  Future loadChildren() async {
    ...
    items[Gender.Male.toString()] = ReturnVisitsByNameModel(header: _getRandomManEmoji(), items: UnmodifiableListView([]));
    items[Gender.Female.toString()] = ReturnVisitsByNameModel(header: _getRandomGirlEmoji(), items: UnmodifiableListView([]));
              ...
              if (rvModel.hasEmptyName) {
                items[rvModel.gender.toString()] = ReturnVisitsByNameModel(
                      ...
                      items: UnmodifiableListView(
                          items[rvModel.gender.toString()].items.toList() + [rvModel]));
              } else {
                ...
                if (items.containsKey(letter)) {
                  items[letter] = ReturnVisitsByNameModel(
                      header: letter,
                      items: UnmodifiableListView(
                          items[letter].items.toList() + [rvModel]));
                } else {
                  items[letter] = ReturnVisitsByNameModel(
                      header: letter, items: UnmodifiableListView([rvModel]));
                }
              }
            }
        
            _items.addAll(
                items.entries.where((i) => i.value.items.isNotEmpty).map((e) => e.value));
            notifyListeners();
          }
