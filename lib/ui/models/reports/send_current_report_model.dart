...
class SendCurrentReportModel extends ChangeNotifier {
  ...
  factory SendCurrentReportModel([TimeService timeService, AppSettingsService appSettings]) {
    ...
    return SendCurrentReportModel._(
      timeService ?? container.resolve<TimeService>(),
  ...
  Future sendReport() async {
    ...
    var endDate = DateTime(DateTime.now().year, _lastReportMonthInt + 1, 1).subtract(Duration(milliseconds: 1));
...
"""$_lastReportMonth Field Service Activity:
...
""",
  ...
  void _loadData() async {
    _dataLoaded = false;
    _lastReportMonthInt = await _appsettings.getSettingInt(AppSettingsService.last_report_sent, DateTime.now().month - 1);
    _lastReportMonth = DateFormat.MMMM(Intl.defaultLocale).format(DateTime(DateTime.now().year, _lastReportMonthInt));
    notifyListeners();
    _dataLoaded = true;
  }
}