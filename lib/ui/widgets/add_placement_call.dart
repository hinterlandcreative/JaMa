...
Future showAddEditVisitModal(
  ...
  return await showMaterialModalBottomSheet(
    ...
    builder: (context, scrollController) => 
      SingleChildScrollView(child: _AddEditVisit(
        visit: visit, 
...
Future showAddSinglePlacementModal(BuildContext context, Function(int count, PlacementType type, String description) onPlacementChanged) async {
  int count = 1;
  PlacementType type;
  String description = "";
...
class __AddEditVisitState extends State<_AddEditVisit> {
    ...
    return Container(
       child: Column(
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                FlutterSwitch(
                  ...
                  showOnOff: true,
                  width: 150,
                  onToggle: (value) => setState(() => widget.visit.type = value ? VisitType.ReturnVisit : VisitType.Study),
                )
              ],
            ),
          ),
          ...
          children: <Widget>[
            ...
            if(widget.isDeleteable) SlideAndFadeTransition(
  ...
  Column buildAddPlacement(BuildContext context) {
    int count = 0;
    PlacementType type;
    String notes = "";
    return Column(
      ...
      children: <Widget>[
        ...
        Row(
          ...
          }, icon: Icon(Icons.close), label: Text("cancel")),
          FlatButton.icon(
            icon: Icon(Icons.add),
            label: Text("add"),
            onPressed: () {
              ...
              widget.visit.placements.add(Placement(
                count: count,
                type: type,
                notes: notes
              ));
          ...
          TreeSelect(
            ...
            onSelectionMade: (nodes) {
              if(nodes != null && nodes.isNotEmpty) {
                ...
                type = PlacementType.values.firstWhere((type) => type.toString().allAfter('.') == typeString); 
  ...
  Node _getPlacements() {
    var possiblities = Node(content: "Placements", children: [
      Node(content: PlacementType.Magazine.toString().allAfter("."), children: [
        ...
        Node(content: "Awake", children: [
          Node(content: "Issue #1")
        ])
      ]),
      Node(content: PlacementType.Brochure.toString().allAfter("."), children: [
        Node(content: "Good News")
      ])
    ]);
